...
def buildModel(hidden_dim, conv_layer, dropout, jk, pool, z_ratio, aggr):
    ...
    Args:
        ...
        conv_layer: number of SAWBNConv.
        pool: pooling function transfer node embeddings to subgraph embeddings.
        z_ratio: see SAWBNConv in impl/model.py. Z_ratio in [0.5, 1].
    ...
    conv = models.EmbZGConv(hidden_dim,
                            ...
                            jk=jk,
                            dropout=dropout,
                            conv=functools.partial(models.SAWBNConv,
                                                   aggr=aggr,
                                                   z_ratio=z_ratio,
                                                   dropout=dropout),
                            gn=True)
    ...
    gnn = models.SAWBN(conv, torch.nn.ModuleList([mlp]),
                       torch.nn.ModuleList([pool_fn1]), models.GraphConvolutionalAutoencoder).to(config.device)
    return gnn
...
def test(pool="size",
    ...
    Args:
        jk: whether to use Jumping Knowledge Network.
        z_ratio: see SAWBNConv in impl/model.py. A hyperparameter of SAWBN.
